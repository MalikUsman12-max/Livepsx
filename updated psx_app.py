...
const LIVE_REFRESH_PROXY_URL = "__LIVE_REFRESH_PROXY_URL__"; // kept for Add Holding only
let liveRefreshInFlight = false;
let autoRefreshTimer = null;

// Reads prices.json from the SAME domain (github.io) — no CORS issue possible.
// Apps Script pushes this file to the repo every hour via the GitHub API.
// The dashboard just reads it like any normal file on the same site.
async function tryLiveRefresh(isAuto){
  ...
  statusEl.textContent = isAuto ? 'Auto-refreshing prices…' : 'Fetching latest prices…';
  statusEl.style.color = 'var(--muted)';

  try {
    // prices.json lives in the same repo, same domain — always works
    const resp = await fetch('prices.json?t=' + Date.now(), { cache: 'no-store' });
    if(!resp.ok) throw new Error(
      resp.status === 404
        ? 'prices.json not found yet — Apps Script trigger may not have run yet. Check your GitHub repo for the file, or wait for the next hourly trigger.'
        : `prices.json fetch failed with status ${resp.status}`
    );
    const data = await resp.json();
    if(!data.prices || Object.keys(data.prices).length === 0){
      throw new Error('prices.json exists but contains no price data');
    }
    ...
    const asOf = data.updated ? new Date(data.updated).toLocaleTimeString() : 'unknown time';
    statusEl.textContent = `✓ ${updated} prices updated (PSX data as of ${asOf}, ${data.count} symbols total). Charts now project from live prices.`;
    statusEl.style.color = 'var(--buy)';
  } catch(err) {
    statusEl.innerHTML = `✗ ${err.message} <a href="https://dps.psx.com.pk/" target="_blank" style="color:var(--accent);">Open PSX directly</a>`;
...
function startAutoRefresh(){
  ...
  // Auto-refresh every 5 minutes — prices.json updates hourly from Apps Script,
  // so checking every 5 min means you always see it within 5 min of it updating.
  autoRefreshTimer = setInterval(() => {
    if(document.visibilityState !== 'visible') return;
    tryLiveRefresh(true);
  }, 300000);
  // Also try immediately on load so you don't wait 5 min for the first refresh
  tryLiveRefresh(true);
}
